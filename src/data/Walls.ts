...
export class Walls {
  ...
  hasWall = (x: number, y: number, dir: Direction): boolean => {
    ...
    case Direction.NORTH:
      if (x < 0 || x >= this.width || y < 0 || y >= (this.height - 1)) { return true; }
      break;
    case Direction.SOUTH:
      if (x < 0 || x >= this.width || y < 1 || y >= this.height) { return true; }
      break;
    case Direction.EAST:
      if (x < 0 || x >= (this.width - 1) || y < 0 || y >= this.height) { return true; }
      break;
    case Direction.WEST:
      if (x < 1 || x >= this.width || y < 0 || y >= this.height) { return true; }
      break;
    }
  ...
  wallIndex = (x: number, y: number, dir: Direction): number => {
    ...
    switch (dir) {
      case Direction.NORTH: return index;
      case Direction.EAST: return index + 1;
      case Direction.SOUTH: return this.wallIndex(x, y - 1, Direction.NORTH);
      case Direction.WEST: return this.wallIndex(x - 1, y, Direction.EAST);
    }
  }
  ...
  wallToCoords = (wall: number): { coordinate: Coordinate, direction: Direction } => 
    ({ 
